...
assert abs(gw2.data[0,0,0].real) < 1e-12
assert max(abs(gw.data[:] - gw2.data[:])) < 1e-12
assert max(abs(gw.data[:] - gw3.data[:])) < 1e-12
...
assert abs(gt.data[0] - gt.data[-1]) < 1e-12

gw2 << Fourier(gt)
gw3 = make_gf_from_fourier(gt)
assert abs(gw2.data[0].real) < 1e-12
assert max(abs(gw.data[:] - gw2.data[:])) < 1e-12
assert max(abs(gw.data[:] - gw3.data[:])) < 1e-12

gw << Fourier(0.3*gt)
assert abs(gw.data[0].real) < 1e-12
...
assert abs(blgt[0].data[0] - blgt[0].data[-1]) < 1e-12

# blgw2 << Fourier(blgt)
blgw3 = make_gf_from_fourier(blgt)

# assert abs(blgw2.data[0].real) < 1e-12
# assert max(abs(blgw.data[:] - blgw2.data[:])) < 1e-12
assert max(abs(blgw.data[:] - blgw3.data[:])) < 1e-12
